...
const HeaderDeliveryCart = ({role,NumberValue}) => {

    const redirectionHome = (role) => {
        if(role==="restaurateur"){
            window.location.href = '/restaurant/home';
        } else if(role==="livreur"){
            window.location.href = '/livreur/home';
        }
    };
    ...
    return(
        <div className="flex-none">
            ...
            <div tabIndex={0} role="button" className="btn btn-ghost btn-circle" onClick={(role)=>redirectionHome(role)} >
                ...
                <span className="badge badge-sm indicator-item">{NumberValue}</span>
                </div>
            </div>
            </div>
        </div>
...
export default HeaderDeliveryCart;